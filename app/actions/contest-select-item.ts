...
/**
 * Phase 8: 選擇目標物品後執行效果
 * 用於對抗檢定獲勝後，需要選擇目標物品的情況
 * Phase 9: 支援攻擊方和防守方選擇目標物品
 */
export async function selectTargetItemForContest(
  ...
  try {
    ...
    if (!contestInfo) {
      ...
      attackerSourceType = contestInfo.sourceType; // 這是攻擊方的技能/物品類型
    }
    ...
    // Phase 9: 根據是否是防守方選擇，從對應的角色身上找到技能或物品
    let source: SkillType | ItemType | null = null;
    let actualSourceType: 'skill' | 'item'; // 實際使用的技能/物品類型（用於效果執行）
    
    // Phase 9: 如果防守方選擇物品，使用傳入的防守方技能/物品信息
    if (isDefenderSelecting && defenderSourceId && defenderSourceType) {
      ...
      // 攻擊方選擇物品時，使用從 contestId 解析出的攻擊方技能/物品信息
    ...
    // Phase 2: 使用統一的通知管理器發送對抗檢定效果事件（選擇目標物品後）
    try {
      // 使用更新後的角色資料獲取技能或物品對象（用於通知）
      ...
      if (attackerSource) {
        // Phase 9: 如果攻擊方獲勝，需要查找防守方使用的技能/物品（用於發送防守方失敗通知）
        ...
        if (!isDefenderSelecting && contestResult === 'attacker_wins') {
          // 攻擊方獲勝：需要查找防守方使用的技能/物品
          // 修復：僅當防守方確實回應時才查找，避免使用前一個對抗的值
          // 檢查防守方的技能/物品是否在當前對抗的時間範圍內使用
          ...
          if (contestStartTime && contestStartTime > 0) {
            ...
            // 查找在當前對抗時間範圍內使用的物品（按 lastUsedAt 排序）
            ...
            // 如果沒有找到在當前對抗時間範圍內使用的技能/物品，則 defenderSkillsForNotification 和 defenderItemsForNotification 保持為 undefined
          }
    ...
    return {
      ...
      message: `目標物品選擇成功：${finalEffectMessage}`,
    };
  } catch (error) {
