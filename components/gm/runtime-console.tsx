...
import type { CharacterData, Stat, Item } from '@/types/character';
...
export function RuntimeConsole({ gameId, characters }: RuntimeConsoleProps) {
  ...
  // Local character state — 從 Server props 初始化，WebSocket 事件增量更新
  const [statsOverrides, setStatsOverrides] = useState<Map<string, Stat[]>>(
    () => new Map(),
  );
  const [itemsOverrides, setItemsOverrides] = useState<Map<string, Item[]>>(
    () => new Map(),
  );

  const characterIds = useMemo(() => characters.map((c) => c.id), [characters]);

  // 合併 Server props + WebSocket 覆蓋的 characters（stats + items）
  const liveCharacters = useMemo(() => {
    if (statsOverrides.size === 0 && itemsOverrides.size === 0) return characters;
    return characters.map((c) => {
      const statsOv = statsOverrides.get(c.id);
      const itemsOv = itemsOverrides.get(c.id);
      if (!statsOv && !itemsOv) return c;
      return {
        ...c,
        ...(statsOv ? { stats: statsOv } : {}),
        ...(itemsOv ? { items: itemsOv } : {}),
      };
    });
  }, [characters, statsOverrides, itemsOverrides]);
  ...
  // 當前 stats / items 快照（供 WebSocket listener 做增量更新的基準）
  ...
  const handleItemsUpdate = useCallback((characterId: string, items: Item[]) => {
    setItemsOverrides((prev) => {
      const next = new Map(prev);
      next.set(characterId, items);
      return next;
    });
  }, []);
