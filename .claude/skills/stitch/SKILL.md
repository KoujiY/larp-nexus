...
1. **先讀 `screen.png`**（視覺分析起點）
   - 這是最重要的一步，必須優先閱讀
   - 對照每個區塊：版面結構、元件位置、色彩、字體層次
   - **不能假設現行實作已接近設計稿**，必須逐區確認差異

2. **再讀 `DESIGN.md`**（設計決策與 token 對照）
   - 讀取設計決策（間距、字體大小、顏色意圖）
   - Stitch 使用 Material Design-like token，需對照本專案 token

3. **最後讀 `code.html`**（HTML 結構語義參考）
   - 理解 Stitch 的結構意圖
   - **禁止直接複製**：使用了不同的 class 系統和元件庫，會破壞現有狀態邏輯

### 3-2：輸出視覺差異清單（**必須在動手前輸出，等待用戶確認**）

讀完三份文件後，**必須輸出以下格式的差異清單**，等待用戶確認後才能開始實作：
