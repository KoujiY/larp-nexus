...
> 狀態：**Phase 1 已拍板（2026-06-13，見「需要拍板的決策點」）— Phase 2 實作中**
...
1. 問題 1（TDD：先寫 executor 時序測試 → 實作 → 手動驗收指引 → 等驗收 → commit）
2. 問題 2 endGame convert-in-place（TDD：先寫 end-game 轉型測試 → 實作 → 手動驗收指引 → 等驗收 → commit）
3. startGame flag-first 重排（TDD：先寫 start-game 順序/回滾測試 → 實作 → 手動驗收指引 → 等驗收 → commit）
4. 知識庫同步（`shared/contest/contest-flow.md`、`gm/game/game-states.md`、`architecture/data-models.md` 的 snapshot 語意與 lifecycle 順序）
5. 結案與封存（本文件 → `docs/archive/`、BACKLOG 劃掉完成項 + D5 改寫 + 殘留事項搬移、USER_GUIDE 檢視、bump version）

## 殘留事項（結案時搬 BACKLOG）

- startGame 殘留視窗：in-flight action（已快取 isActive=false）的 Baseline 寫入在複製讀取後落地 → silent invisible（資料留在 Baseline、賽後可見）。徹底閉合需 Character collection 條件欄位 + migration，已評估不成比例，僅記載語意
- （結案時補充）
