...
- **問題**：目前登入頁面與狀態提示頁（loading、error、empty）的設計未經過 Stitch 精修
- **設計稿**：`docs/specs/Stitch/GM_dashboard_refactor_4.19_login_status/` 與 `docs/specs/Stitch/GM_dashboard_refactor_4.19_user_login_page/`
- **注意事項**：
  - 設計稿分淺色/深色版本，但實作上統一使用瀏覽器偏好主題
  - 狀態頁有三種狀態（loading、error、empty），文案需改為通用/低調風格
- **影響範圍**：`app/(gm)/login/`、共用狀態元件
...
| 順序 | 項目 | 規模 | 理由 |
|------|------|------|------|
| 1 | #0-1 劇本頁空狀態修正 | 小 | 最小改動，熱身 |
| 2 | #0-2 登入頁 & 狀態頁重新設計 | 中 | Stitch 流程，純 UI 改版 |
