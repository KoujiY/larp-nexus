...
- **問題**：目前的空狀態樣式不正確，使用了一般的空狀態元件
- **期望**：使用虛線外框 + 中央 `+` icon + 按鈕的標準空狀態設計（參考 `components/gm/dashed-add-button.tsx`）
- **影響範圍**：`app/(gm)/games/page.tsx`、`app/(gm)/games/[gameId]/page.tsx`

### 0-2. 頁面 Loading 狀態

- **問題**：GM 端和玩家端的路由切換（如側邊欄導航）有明顯延遲，但沒有 loading 回饋，使用者體感卡頓
- **根因**：全站沒有任何 `loading.tsx`。每個 async Server Component 在等待 DB 資料時，頁面完全凍結無回饋
- **期望**：為所有 async 路由新增 `loading.tsx`，讓使用者在資料載入期間看到 loading 畫面
- **盤點結果**：客戶端操作（按鈕觸發的 Server Action）已有 85% loading 覆蓋率，不需額外處理
- **需新增 `loading.tsx` 的路由**：

  | 路由 | async 呼叫數 | 說明 |
  |------|-------------|------|
  | `app/(gm)/games/loading.tsx` | 1 | GM 劇本列表 |
  | `app/(gm)/games/[gameId]/loading.tsx` | 2 | GM 劇本詳情 + 角色列表 |
  | `app/(gm)/games/[gameId]/characters/[characterId]/loading.tsx` | 3 | GM 角色編輯（最高延遲風險） |
  | `app/(gm)/profile/loading.tsx` | 1 | GM 個人檔案 |
  | `app/c/[characterId]/loading.tsx` | 1 | 玩家角色卡 |
  | `app/(player)/g/[gameId]/loading.tsx` | 1 | 玩家世界觀 |

- **實作方式**：Next.js `loading.tsx` 自動作為路由的 Suspense fallback，資料載入期間顯示 loading UI，載入完成後無縫切換為完整頁面。側邊欄等共享 layout 不受影響，只有頁面內容區域顯示 loading。
...
| 順序 | 項目 | 規模 | 理由 |
|------|------|------|------|
| 1 | #0-1 劇本頁 / 角色列表空狀態修正 | 小 | 最小改動，熱身 |
| 2 | #0-2 頁面 Loading 狀態 | 小 | 改善使用者體驗，不涉及業務邏輯 |
| 3 | #1 Slogan 欄位 | 小 | 小型 schema + UI 變更 |
| 4 | #2 圖片上傳系統 | 中 | API route + 共用元件 |
| 5 | #5 預設事件系統 | 中 | 複用現有 actions，獨立新資料結構 |
