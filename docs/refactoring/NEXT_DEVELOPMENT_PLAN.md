...
1. **基礎設施**（先確定整條鏈路能跑通）
   - 安裝 `@playwright/test` 與 `mongodb-memory-server`（pnpm，devDep）
   - 下載 Playwright browser binary：`pnpm exec playwright install chromium`
   - 撰寫 `lib/websocket/pusher-server.e2e.ts` 與 `lib/websocket/pusher-client.e2e.ts` stub 模組（in-memory event bus + server↔client IPC 機制）
   - `next.config.ts` 加 webpack alias（E2E env 啟用時替換 Pusher 模組）
   - `playwright.config.ts` + `e2e/global-setup.ts`（啟 mongodb-memory-server）+ `e2e/global-teardown.ts`
   - `app/api/test/login/route.ts`（`E2E=1` env guard，非 E2E 回 404）
   - `package.json` 新增 `test:e2e` script（`E2E=1 playwright test`）
