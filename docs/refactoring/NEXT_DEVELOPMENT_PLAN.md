...
E2E 跑起來時由 Playwright orchestrate 四個 process，開發者只需一條指令（`npm run test:e2e`）：

| Process | 角色 | 啟動者 |
|---|---|---|
| Playwright test runner | 跑 spec、控制 Chromium、收拾周邊 | `npm run test:e2e` |
| Next.js server（test mode）| 被測物，與 production 同一份 code | Playwright `webServer` config |
| soketi | Pusher 協定相容的本地 WebSocket server | Playwright `globalSetup` |
| Chromium | 真實瀏覽器分頁 | Playwright 每個 test 自己開 |

加上一個寄生於 Next.js process 的 `mongodb-memory-server` subprocess（真 mongod、非持久化、按需啟動）。
...
| 測試資料庫 | **`mongodb-memory-server`** | 真 mongod binary，非持久化，零外部依賴，per-worker database 做隔離 |
| Pusher 替身 | **soketi**（本地 Pusher 相容 server）| 利用 Pusher SDK 原生 self-hosted 能力，走真 WebSocket path，離線可跑，零 rate limit |
| 登入 bypass | **test-only login API**（`/api/test/login`，`E2E=1` env guard）| 避免 E2E 依賴真 SMTP；production build 此路由 throw |
...
**`mongodb-memory-server` 的定位說明**：名稱中的「memory」指的是資料存放策略（非持久化 tmpfs / 臨時目錄），不是 mock 實作。它實際上下載並 spawn 真正的 `mongod` binary，行為與 Atlas 完全一致（包括 index、transaction、aggregation）。第一次 npm install 會快取 binary，後續啟動約 1–2 秒。

**soketi 的定位說明**：soketi 是 Pusher 協定相容的開源 WebSocket server。透過設定 `PUSHER_HOST` / `PUSHER_PORT` 等環境變數，`pusher-server.ts` / `pusher-client.ts` 會改連到本地 soketi 而非 Pusher cluster——這是 Pusher SDK 官方支援的 self-hosted 模式，不是 hack。production 環境不設這些 env var 即維持原本行為。

### 需要對 production code 的最小變動

E2E 基礎設施本身需要對兩個檔案加上「env 觸發的 self-hosted」分支。這不是測試 hack，而是把 Pusher SDK 原生就有的 self-hosted 能力暴露出來：

1. `lib/websocket/pusher-server.ts` — 讀 `PUSHER_HOST` / `PUSHER_PORT`，若存在則傳 `host` / `port` / `useTLS: false` 給 `new Pusher(...)`
2. `lib/websocket/pusher-client.ts` — 讀 `NEXT_PUBLIC_PUSHER_HOST` / `NEXT_PUBLIC_PUSHER_PORT`，若存在則傳 `wsHost` / `wsPort` / `forceTLS: false` 給 `new Pusher(...)`

production 環境這兩組 env var 皆不設定 → code path 完全不變，仍走 cluster + TLS 連真 Pusher。
...
│   ├── db-fixture.ts                   # per-worker mongodb-memory-server lifecycle
...
├── flows/                          # 完整功能流程，PR 合併前跑
│   ├── character-creation.spec.ts
│   ├── skill-use.spec.ts
│   ├── contest.spec.ts
│   ├── item-operations.spec.ts
│   └── broadcast.spec.ts
├── global-setup.ts                 # 啟 soketi、初始化共享資源
├── global-teardown.ts              # 關 soketi、清 cache
└── soketi.json                     # soketi 設定檔
```
...
1. **基礎設施**（先確定整條鏈路能跑通）
   - 安裝 `@playwright/test`、`mongodb-memory-server`、`soketi`
   - `pusher-server.ts` / `pusher-client.ts` 加 host/port env 支援
   - `playwright.config.ts` + `e2e/global-setup.ts` + `e2e/global-teardown.ts`
   - `app/api/test/login/route.ts`（env guard）
   - `package.json` 新增 `test:e2e` script
2. **Fixtures**（可重複使用的測試樂高積木）
   - `db-fixture.ts`（per-worker mongodb-memory-server）
   - `auth-fixture.ts`（GM / Player 注入 session）
   - `seed-fixture.ts`（劇本/角色/道具 seed builder）
   - `helpers/wait-for-*.ts`
3. **smoke 層**（2 個 spec，先確認 fixture 可用）
4. **flows 層**（6 個 spec，逐一加）

### 影響範圍

- **新增**：`e2e/`、`playwright.config.ts`、`e2e/global-setup.ts`、`e2e/global-teardown.ts`、`app/api/test/login/route.ts`、`e2e/soketi.json`、`.env.test`
- **修改（最小）**：`lib/websocket/pusher-server.ts`、`lib/websocket/pusher-client.ts`（加 host/port env 支援）、`package.json`（devDependencies + scripts）、`.gitignore`（mongodb-memory-server 的 cache 目錄）
- **不影響**：所有 server actions、hooks、React components、types。E2E 走的是與 production 相同的 code path，只是把外部服務換成本地替身

### 技術考量

- **Windows 相容性**：`mongodb-memory-server` 與 `soketi` 皆支援 Windows。soketi 建議以 Docker container 或 npx 方式執行，避免手動安裝
...
- `npm run test:e2e` 一條指令可啟動全套（Playwright + Next.js + soketi + mongodb-memory-server + Chromium），跑完自動清乾淨
- smoke 層全綠、flows 層全綠
- **可完全離線執行**（無任何外部依賴）
- production `npm run build` 不受影響（E2E 相關檔案不進 build）
...
| 8 | #6 E2E 測試建置 | 中 | 純地端；Playwright + soketi + mongodb-memory-server 三層（smoke/flows/nightly）|
