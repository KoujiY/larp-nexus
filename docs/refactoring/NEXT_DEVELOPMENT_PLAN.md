...
原本無 PIN 角色跳過入口頁面直接顯示角色卡，導致無法透過 Game Code 切換 Runtime 模式、橫幅和重新鎖定按鈕也不會顯示。修正為所有角色統一經過入口頁面：

- `useLocalStorageUnlock` 移除 `hasPinLock` 旁路，所有角色統一走 localStorage
- Gate 從 `hasPinLock && !isUnlocked` 改為 `!isUnlocked`
- `pin-unlock.tsx` 新增 `hasPinLock` prop：無 PIN 時隱藏 PIN 輸入區，其餘 UI（Game Code + 按鈕）一致
- `character-mode-banner.tsx` 移除 `hasPinLock` 門檻，preview/runtime 橫幅對所有角色統一顯示
