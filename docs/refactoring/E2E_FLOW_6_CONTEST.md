...
- 互斥選擇 + opponentMax 上限 + 預設 0 不可選（#6.5）
...
| #6.5 互斥選擇 + opponentMax | 2 角色 + 1 攻擊技能（opponentMaxItems=2, opponentMaxSkills=1）+ 多防禦資源 | ✅ | active |
...
## #6.5 互斥選擇 + opponentMax 上限 + 預設 0 不可選
...
- 角色 A（attacker）：
  ...
  skills: [{
    ...
    contestConfig: {
      relatedStat: 'str',
      opponentMaxItems: 2,     // ← 允許最多 2 個道具
      opponentMaxSkills: 1,    // ← 允許最多 1 個技能
      tieResolution: 'attacker_wins',
    },
    effects: [{ type: 'stat_change', target: 'other', statKey: 'hp', value: -10 }],
...
- 角色 B（defender）：
  ...
  items: [
    ...
    { id: 'item-d3', name: '防具三', type: 'tool', checkType: 'contest', contestConfig: { relatedStat: 'str' }, tags: [], quantity: 1 },
  ]
  skills: [
...
2. pageB: 等待 `contestDialog` 出現
3. **斷言 — 三個道具和兩個技能全部可見**（無 combat tag 限制，checkType 皆匹配）

4. **測試 opponentMaxItems 上限**：
   - 選 `item-d1` ✅ → `selectedItems.length === 1`
   - 選 `item-d2` ✅ → `selectedItems.length === 2`
   - 點擊 `item-d3` → **被擋住**，顯示 warning「最多只能選擇 2 個物品」
   - **斷言**：`item-d3` 的 checkbox 未被勾選

5. **測試互斥切換（items → skills）**：
   - 目前已選 item-d1 + item-d2
   - 點擊 `skill-d1` → **道具選擇自動清空**（contest-response-dialog.tsx:137 `setSelectedItems([])`）
   - **斷言**：`item-d1` 和 `item-d2` 的 checkbox 都已取消
   - **斷言**：`skill-d1` 的 checkbox 已勾選

6. **測試 opponentMaxSkills 上限**：
   - 目前已選 skill-d1
   - 點擊 `skill-d2` → **被擋住**，顯示 warning「最多只能選擇 1 個技能」
   - **斷言**：`skill-d2` 的 checkbox 未被勾選

7. pageB: 保持 skill-d1 選中 → 按「回應」

### Phase C — 結果驗證

8. 等待 result + effect 事件（A 力量 60 > B 力量 50 → attacker_wins）
9. B 的 HP = 90（100 - 10）
10. B 的 `skill-d1` usageCount += 1
...
9. **防禦資源選擇是 items OR skills 互斥**：不能混搭。選 items 時 skills 自動清空，反之亦然（contest-response-dialog.tsx:122, 137）。#6.5 專門驗證此行為
