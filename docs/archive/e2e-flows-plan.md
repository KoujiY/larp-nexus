...
clickSaveBar(page, { timeout?, interval? })                                     ✅ e2e/helpers/click-save-bar.ts
...
**對策**：在 `router.refresh()` 之後、下一步互動之前等待 **Sonner toast 出現並消失**作為穩定信號。toast 自動消失需 ~4 秒（Sonner 預設），此時 `router.refresh()` 的重渲染必然已完成。
...
// 方法 A：save 後第一次等待（toast 尚未被其他程式碼 await）
await expect(page.locator('[data-sonner-toast]')).toBeVisible({ timeout: 5000 });
await expect(page.locator('[data-sonner-toast]')).not.toBeVisible({ timeout: 10000 });

// 方法 B：前面已有 waitForToast()，只需等 toast 消失
await expect(page.locator('[data-sonner-toast]')).not.toBeVisible({ timeout: 10000 });
```

**注意**：~~1000ms 是經驗值~~（已廢棄）。toast 消失是 deterministic 信號，比固定 timeout 更可靠。若 save 沒有 toast（見規則 27），才考慮用 `waitForTimeout(500)` 作為 fallback。
...
### 規則 33：SaveBar「全部儲存」按鈕 — `clickSaveBar` helper 取代 Playwright click
...
**舊方案（已廢棄，有 TOCTOU）**：`waitForFunction` + `evaluate` 分開呼叫 → 按鈕可能在兩次呼叫之間被 detach。

**現行對策**：使用 `e2e/helpers/click-save-bar.ts` 共享 helper，將「找按鈕 + 檢查 `isConnected` + `click()`」合併在單一 `page.evaluate()` 內，消除 TOCTOU gap：

```typescript
import { clickSaveBar } from '../helpers/click-save-bar';

// ✅ 單一 evaluate retry loop，原子性保證
await clickSaveBar(page);
// 可選：加長 timeout（預設 10s）
await clickSaveBar(page, { timeout: 15000 });
```

**所有 spec 中的 `saveAllBtn.click()` 都已替換為 `clickSaveBar(page)`**。禁止直接用 Playwright locator 點擊 SaveBar 按鈕。
...
## E2E Flaky Test 根治策略（Phase 5 穩定化總結）

> 以下三個方法源自 Phase 5 flows spec 開發期間，對 #3.3、#4.2、#4.3、#10.3、#10.5 五個 flaky test 的根因分析。三個方法互補，系統性套用後消除了所有已知 flakiness。

### 方法 1：`retries: 1`（兜底層）

`playwright.config.ts` 設定 `retries: process.env.CI ? 2 : 1`。本地開發允許一次重試，CI 允許兩次。

**不是萬靈丹**：retry 只是兜底，不應依賴它讓不穩定的測試「碰運氣通過」。如果某個 test 需要 retry 才能通過，應進一步套用方法 2 或 3 修正根因。

### 方法 2：用穩定信號取代 `waitForTimeout`

`waitForTimeout(N)` 是在「猜」等待時間：太短 → flaky，太長 → 浪費時間。應改用明確的完成信號。

| 場景 | 舊寫法 | 新寫法 |
|------|--------|--------|
| `router.refresh()` 後等重渲染 | `waitForTimeout(1000)` | 等 toast 出現+消失（見規則 13） |
| Server action 副作用（DB 變更） | `waitForTimeout(2000)` | `expect.poll()` 輪詢 DB 斷言 |
| React effect chain 傳播 | `waitForTimeout(500)` | **保留**（無更好信號，但需註解說明原因） |

**`expect.poll` 用法**（server-side effect 等待）：

```typescript
// ❌ 固定等待 — 太短 flaky、太長浪費
await pageA.waitForTimeout(2000);
const runtimeB = await dbQuery('character_runtime', { refId: charB._id });
expect(bHp.value).toBe(70);

// ✅ polling — 效果完成立即通過，最多等 10 秒
...
**剩餘合理的 `waitForTimeout(500)` 使用場景**（需逐一註解）：
- dirty state 跨多層 component propagation（`isDirty` → `useEffect` → `onDirtyChange` → parent `setState`，需 2+ render cycle）
- `pageerror` 等負面斷言（等待「沒有錯誤發生」，無正面信號可 poll）

### 方法 3：AnimatePresence 元素用 `page.evaluate` 取代 Playwright locator

Playwright locator 的 actionability check 分兩步：①找元素 ②執行動作。Framer Motion `AnimatePresence` 的 spring 動畫可能在這個 gap 中 detach DOM 節點，導致 `click()` / `fill()` timeout。

**核心原則**：將「找 + 檢查 `isConnected` + click」合併為 **單一 `page.evaluate()` 呼叫**，消除 TOCTOU gap。

```typescript
// ❌ Playwright locator — 有 TOCTOU 風險
...
**適用範圍**：所有 `AnimatePresence` 包裹的互動元素，目前主要是 `StickySaveBar`。Dialog 內部因 React re-render 而 detach 的元素（如 Select trigger、確認按鈕）也適用同樣原則（見規則 31/32）。

**不適用範圍**：Portal 元素（如 Radix Select 的 `[role="option"]`）不受 `AnimatePresence` 影響，應繼續用 Playwright locator（因為 `HTMLElement.click()` 不觸發 pointer events，Radix Select 的 `onPointerUp` handler 不會執行）。

### 方法適用判斷流程

```
Test 偶發失敗
  ├─ Error: timeout / detached → 方法 3（evaluate 取代 locator）
  ├─ Error: 斷言值不符（DB 還沒更新）→ 方法 2（polling 取代 timeout）
  ├─ Error: 狀態丟失（dirty 被 reset）→ 檢查 router.refresh() race → 方法 2（toast wait）
  └─ 以上都不是 → 方法 1（retry 兜底）+ 開 issue 追蹤
```

---

### 規則 35：`<button>` 在 `<form>` 內必須有 `type="button"`

HTML `<button>` 預設 `type="submit"`。若 icon button（如「編輯」「刪除」）放在 `<form>` 內但不該觸發 submit，**必須**設 `type="button"`。

**實際案例**：`IconActionButton` 元件的 `<Button>` 未設 `type="button"`，導致 GM 在 secrets tab 點「編輯」時意外觸發 `<form onSubmit>`，引發 `save()` 和 `saveAll()` 的 race condition，使 `autoRevealCondition` 資料丟失。這是 production code 層級的 bug，不是 test 層級的問題。

```tsx
// ❌ 預設 type="submit" → 意外觸發 form submission
<Button variant="ghost" size="icon" onClick={onClick}>
  {icon}
</Button>

// ✅ 明確 type="button" → 只觸發 onClick
<Button type="button" variant="ghost" size="icon" onClick={onClick}>
  {icon}
</Button>
```

### 規則 36：Strict mode violation — 用 `main` scope 排除導覽列重複文字

`getByText('某文字')` 可能匹配到多個元素（例如麵包屑導覽列和 `<main>` 內都有同樣文字）。Playwright strict mode 會拋出 `resolved to 2 elements` 錯誤。

**對策**：用 `page.locator('main').getByText(...)` 限縮搜尋範圍。

```typescript
// ❌ 匹配到 main + breadcrumb 兩個元素
